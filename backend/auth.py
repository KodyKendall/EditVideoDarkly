...
class UserRegistration(SQLModel):
    ...
    #what will we ask from the user to get them to register
    username: str
    email: str
    password: str
...
# class DuplicateValue(AuthException):
#     def __init__(self, entity_name: str, entity_field: str, entity_value: str):
#         super().__init__(
#             error="duplicate_value",
#             description=f"Duplicate {entity_field} '{entity_value}' for {entity_name}.",
#         )
...
def register_new_user(
    ...
    existing_user = session.exec(
        select(UserInDB).where((UserInDB.username == registration.username) | (UserInDB.email == registration.email))).first()

    if existing_user:
        field = "username" if existing_user.username == registration.username else "email"
        raise HTTPException(
            status_code=422,
            detail={
                ...
                "entity_name": "User",
                "entity_field": field,
                "entity_value": getattr(registration, field),
            },
        )
